...
export function SearchAllDialog() {
  ...
  const { isLoading, data } = useSearchPage(open ? searchValue : undefined);
  ...
  const GroupPage = ({
    title, pages, isSearchByValue = false, isPagesToday = false,
  }) => {
    ...
    function handle2Light(title: string) {
    ...
    if (data && Object.keys(data).length > 1) {
      ...
      return (
        <Col gap={ 3 }>
          <GroupPage
            isPagesToday
            pages={ data.pagesToday }
            title='Today'
          />
          <GroupPage
            pages={ data.pagesOfYesterday }
            title='Yesterday'
          />
          <GroupPage
            pages={ data.pagesOneWeekAgo }
            title='Past week'
          />
          <GroupPage
            pages={ data.pagesOneMonthAgo }
            title='Past 30 days'
          />
          <GroupPage
            pages={ data.pagesOlder }
            title='Older'
          />
        </Col>
      );
