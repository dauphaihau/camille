...
import { updateUserTrackingSchema } from 'lib/validations/user';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'PATCH') {
    try {
      const body = updateUserTrackingSchema.parse(req.body);
      const userId = req.query.userId;

      if (!body.lastAccessWorkspaceId || typeof userId !== 'string') {
